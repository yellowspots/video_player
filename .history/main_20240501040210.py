...
def create_video(video: dict):
    ...
    video_url = video.get('video_url', '')  # Assuming you've added this column to your database
    upload_date = video.get('upload_date')
    view_count = video.get('view_count', 0)
    rating_average = video.get('rating_average', 0.0)
    theme_id = video.get('theme_id')
    category_ids = video.get('category_ids', [])  # Extracting category IDs
...
def create_tiktok_video(share_link: str):
    ...
    preview_image_url = data['preview_image']
