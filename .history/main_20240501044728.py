...
@app.get("/videos/")
def get_videos(request: Request, theme_id: Optional[int] = None, search_query: str = '', sort: str = 'upload_date'):    conn = get_db_connection()
    cur = conn.cursor()
    query = f"""
    SELECT v.video_id, v.title, v.description, v.preview_image_url, v.video_url FROM Videos v
